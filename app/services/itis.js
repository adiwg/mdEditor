...
const proxy =
  ...
  '&fl=hierarchySoFarWRanks,hierarchyTSN,kingdom,rank,vernacular,tsn,nameWOInd,usage,taxonAuthor,updateDate';
...
export default Service.extend({
  ...
  sendQuery(searchString, kingdom, limit = 50) {
    ...
    const titleized = titleize(searchString.replace(/(-)/g, '#')).replace(
      /( |#)/g,
      '*'
    );
  ...
  parseDoc(doc) {
    ...
    const taxonomy = this.parseRanks(
      ranks,
