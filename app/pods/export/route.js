...
export default Route.extend(ScrollTo, {
  ...
  setupController(controller, model) {
    ...
    defineProperty(this.controller, 'hasSelected', computed(
      'model.records.0.@each._selected', 'model.records.1.@each._selected', 'model.records.2.@each._selected', 'model.settings._selected', 'store',
      function() {
    ...
    defineProperty(this.controller, 'hasSelectedRecords', computed(
      'model.records.0.@each._selected', 'store',
      function() {
        return this.store.peekAll('record').filterBy('_selected').get(
          'length') > 0;
