...
export default class SettingsRoute extends Route {
  ...
  @action
  clearLocalStorage() {
    const keepSettings = Boolean(this.settings.data?.keepSettings);
    const data =
      keepSettings && this.settings.data
        ? this.settings.data.serialize({ includeId: true })
        : null;

    window.localStorage.clear();

    if (keepSettings && data?.data?.id) {
      const storageKey = `settings-${data.data.id}`;
