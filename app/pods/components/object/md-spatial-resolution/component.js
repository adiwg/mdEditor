...
export default Component.extend(Validations, {
    ...
    if (model) {
      once(this, function () {
        set(
          model,
          'measure',
          get(model, 'measure') !== undefined ? get(model, 'measure') : {},
        );
      });
    }
  },
  ...
  levelDisabled: computed('model.scaleFactor', 'measurePresent', function () {
    ...
    return (
      (!isEmpty(scaleFactor) && !Number.isNaN(scaleFactor)) ||
      this.measurePresent
    );
  }),
  measure: alias('model.measure'),
  measureDisabled: computed('model.{scaleFactor,levelOfDetail}', function () {
    let scaleFactor = this.get('model.scaleFactor');
    return (
      (!isEmpty(scaleFactor) && !Number.isNaN(scaleFactor)) ||
