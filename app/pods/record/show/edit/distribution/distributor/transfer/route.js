...
export default class TransferRoute extends Route.extend(ScrollTo) {
  ...
  setupModel() {
    ...
    let distributor =
      distribution && distributorId && isArray(distribution.distributor)
        ...
        : undefined;
    let transfer =
      distributor && transferId && isArray(distributor.transferOption)
        ? A(distributor.transferOption).objectAt(transferId)
        : undefined;
  ...
  deleteTransfer(id) {
    ...
    )[this.controller.distributionId].distributor[this.controller.distributorId]
      .transferOption;
