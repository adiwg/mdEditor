...
module('Integration | Component | object/md-schema/form', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    'Title|URL|Description|Type|Select|the|record|type|for|schema.|Apply|Globally?|No|Yes|');
    ...
    assert.equal(this.element.textContent.replace(/[ \s\n]+/g, '|').trim(),
    '|Title|URL|Description|Type|Select|the|record|type|for|schema.|Apply|Globally?|No|Yes|template|block|text|');
  });
});
