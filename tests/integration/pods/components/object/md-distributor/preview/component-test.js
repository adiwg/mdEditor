...
module('Integration | Component | object/md distributor/preview', function (
  hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(hbs `{{object/md-distributor/preview item=this.distributor}}`);

    assert.equal(this.element.textContent.replace(/[\s\n]+/g, '|').trim(), '|role|(|Contact0|)|');

    // Template block usage:
    await render(hbs `
      {{#object/md-distributor/preview class="testme" item=this.distributor}}
        template block text
