...
module('Integration | Component | object/md spatial info', function (hooks) {
  ...
  test('it renders', async function (assert) {
      ...
      spatialReferenceSystem: [
        ...
        },
        {
          ...
          referenceSystemIdentifier: {
            ...
            namespace: 'UTM',
            description:
              'Universal Transverse Mercator Zone 10 Seattle, Washington',
          },
        },
        {
    ...
    await render(
      hbs`{{object/md-spatial-info profilePath="foobar" model=model}}`
    );
    ...
    assert.equal(
      this.element.textContent.replace(/[\s\n]+/g, '|').trim(),
      '|Spatial|Representation|Type|No|Spatial|Reference|System|found.|Add|Spatial|Reference|System|No|Spatial|Resolution|found.|Add|Spatial|Resolution|No|Raster|Description|found.|Add|Raster|Description|template|block|text|',
      'block'
    );
  });
