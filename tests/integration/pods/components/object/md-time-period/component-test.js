...
module('Integration | Component | object/md time period', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`{{object/md-time-period profilePath="foobar" model=model.firstObject}}`
    );
    ...
    assert.equal(
      ...
      moment(date).format('YYYY-MM-DD HH:mm:ss') +
        '|2016-12-31 00:00:00|identifier|description|periodName0|periodName1|9|1|1|1|1|1|1',
      'input values'
    );

    await render(
      hbs`{{object/md-time-period profilePath="foobar" model=model.lastObject}}`
    );
    ...
    assert.equal(
      input1,
      '||identifier|description|periodName0|periodName1|||||||',
      'geologic input values'
    );
