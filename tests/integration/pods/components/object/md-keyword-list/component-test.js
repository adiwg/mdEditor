...
module('Integration | Component | object/md keyword list', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    await render(hbs`{{object/md-keyword-list model=this.model profilePath="foobar"}}`);

    assert.equal(find('ul').textContent
      .replace(/[ \n]+/g, '|')
      .trim(), '|Delete|foo1|Delete|bar1|');

    await render(hbs`{{object/md-keyword-list model=this.model readOnly=false profilePath="foobar"}}`);
