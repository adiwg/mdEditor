...
module('Integration | Component | object/md process step', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(hbs`{{object/md-process-step profilePath="foobar" model=this.step}}`);

    assert.equal(this.element.textContent.replace(/[\s\n]+/g, '|').trim(),
      "|Step|ID|Description|Description|Step|Sources|1|Add|#|Description|0|Delete|Step|Products|1|Add|#|Description|0|Delete|Processors|2|Add|#|Role|Contacts|0|role|×|Select|one|or|more|contacts|Delete|1|role|×|Select|one|or|more|contacts|Delete|Step|Reference|2|Add|OK|#|Title|0|title0|More...|Delete|1|title1|More...|Delete|Time|Period|Dates|Precision|Day|Start|Date|End|Date|Pick|Fiscal|Year|Pick|a|Fiscal|Year|Identifier|Description|Description|No|Time|Period|Name|found.|Add|Time|Period|Name|Interval|Interval|Amount|Time|Unit|Choose|unit|of|time|Duration|Years|Months|Days|Hours|Minutes|Seconds|Scope|Select|type|of|resource.|"
    );

    // Template block usage:
    await render(hbs`
      {{#object/md-process-step profilePath="foobar" model=this.step}}
        template block text
