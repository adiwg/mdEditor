...
module(
  'Integration | Component | object/md taxonomy/collection/voucher',
  function (hooks) {
    ...
    test('it renders', async function (assert) {
      ...
      await render(
        hbs`{{object/md-taxonomy/collection/voucher profilePath="foobar" model=model}}`
      );

      assert.equal(
        this.element.textContent.replace(/[\s\n]+/g, '|').trim(),
        '|Specimen|Repository|Role|custodian|?|×|Contacts|'
      );
