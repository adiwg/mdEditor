...
module('Integration | Component | input/md markdown area', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`{{input/md-markdown-area value=markdownValue maxlength=10 required=false}}`
    );

    assert.equal(
      find('.md-markdown-editor .length.md-error').textContent,
      'length: 15',
      'maxlength ok'
    );
    ...
    assert.equal(
      find('.md-markdown-editor')
        .innerText.replace(/[ \n\s]+/g, '')
        .trim(),
      '||||Entertext,Markdownissupported.​length:0100:0templateblocktext',
      'block'
    );
  });
});
