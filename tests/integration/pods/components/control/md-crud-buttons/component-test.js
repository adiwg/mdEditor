...
module('Integration | Component | control/md crud buttons', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`{{control/md-crud-buttons allowCopy=true allowDelete=true}}`
    );
    await triggerEvent('.md-crud-buttons', 'mouseenter');

    assert.equal(
      find('.md-crud-buttons').textContent.replace(/[ \n]+/g, '|'),
      '|Copy|Delete|'
    );
    ...
    assert.equal(
      find('.md-crud-buttons').textContent.replace(/[ \n]+/g, '|'),
      '|Save|Cancel|Copy|template|block|text|',
      'block, doSave'
    );

    assert.equal(
      find('.md-crud-buttons .btn-success').disabled,
      true,
      'save disabled'
    );
  });
