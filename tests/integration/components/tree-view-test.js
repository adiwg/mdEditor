...
module('Integration | Component | tree view', function (hooks) {
  ...
  test('it renders and expands', async function (assert) {
    ...
    assert.equal(
      find('.tree-trunk').innerText.replace(/[\s\n]+/g, '|'),
      '|bar1label|foo1label'
    );

    assert.ok(
      find('.tree-leaf').classList.contains('tree-highlight'),
      'selected leaf highlighted'
    );

    assert.equal(
      findAll('.tree-leaf .expand-icon').length,
      1,
    ...
    assert.equal(
      find('.tree-trunk').innerText.replace(/[\s\n]+/g, '|'),
      '|bar1label|foo1label|foo2label'
    );

    await click(findAll('.tree-leaf')[1]);

    assert.equal(
      findAll('.tree-leaf.tree-highlight').length,
      2,
      'node selected'
    );
  });
