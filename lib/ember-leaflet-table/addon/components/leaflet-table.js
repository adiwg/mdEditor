...
export default Component.extend(ResizeAware, {
  ...
  init() {
    ...
    // Bound for callbacks passed to child components (replacing {{action}}).
  ...
  splitFeatures(json) {
    ...
    if (json.type === 'FeatureCollection') {
      json.features.forEach((fea) => {
        ...
        if (fea.geometry.type === 'GeometryCollection') {
          ...
          features.pushObject(fea);
        }
      });
    } else {
      if (json.geometry) {
        features.pushObject(json);
      }
    }
    ...
    features.forEach((fea) => {
      ...
      if (fea.properties.id === id) {
        delete fea.properties.id;
      }
  ...
  // },
  parseJSON(file) {
    let data = new TextDecoder().decode(file.data);
        ...
        function (err, json) {
          if (err) {
            reject(
              `Failed to parse file: ${
                file.name
              }. Is it valid delimited ${type.toUpperCase()}?`
            );
            return;
          }
          resolve(json);
      ...
      shapefile
        ...
        .then((source) => {
          return source.read().then(function next(result) {
            ...
            vertices += geojsonCoords(result.value).length;
            if (vertices > 5000) {
              throw `The import has a limit of 5000 coodinates(vertices). Try simplifying your features.`;
            }
  ...
  readData(file) {
      ...
      switch (ext) {
        ...
        default:
          data = false;
          assert(`File extension "${ext}" not implemented.`);
      }
      ...
      if (data) {
        data
          .then((json) => {
            ...
            this.get('layers').pushObjects(split);

            get(this, 'flashMessages').success(
              `${split.length} features imported!`
            );
          })
          .catch((reason) => {
          ...
          .finally(() => {
            ...
            $('.map-file-picker input:file').val('');
          });
      } else {
        get(this, 'flashMessages').danger('No features imported!');
      }
  },
  ...
  setup(m) {
    ...
    if (layers.length) {
      map.fitBounds(L.geoJson(layers).getBounds());
    }
  },
  ...
  deleteFeature(row) {
    ...
    this.get('layers').removeObject(row);
