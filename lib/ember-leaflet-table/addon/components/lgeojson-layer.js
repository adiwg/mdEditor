...
export default Layer.extend({
  ...
  featureAdded: on('init', observer('geoJSON', function () {
    let json = this.geoJSON;
    if(!json.id) {
      json.set('id', uuidV4());
    }
  ...
  className: computed('geoJSON.state', function () {
    let layerGroup = this._layer;
    let s = this.geoJSON.state;
  ...
  formVisible: on('init', observer('geoJSON.form',
    function () {
      let form = this.geoJSON.form;
