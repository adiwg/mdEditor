...
export default Component.extend({
  ...
  overLimit: computed('actualTotal', 'limit', function () {
    return this.actualTotal > this.limit;
  }),
   ...
   */
  searchResult: computed('exactMatch', 'limit', 'model', 'searchString', function () {
    let pattern = this.searchString;
    let result = [];
    let limit = this.limit;

    if(pattern.length > 0) {
      result = this.searchTree(this.model, this.searchString).sortBy('label');
