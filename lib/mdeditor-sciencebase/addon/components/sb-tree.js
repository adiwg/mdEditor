...
export default Tree.extend({
  ...
  handleDrop(obj, opts) {
    if (
      obj.model?.id &&
      opts.target.model?.id &&
      obj.model?.id === opts.target.model?.id
    ) {
      return;
    }

    let record = obj.model?._record;
    ...
    let newParentRecord = newParent.model?._record;
