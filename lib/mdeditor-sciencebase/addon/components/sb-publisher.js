import classic from 'ember-classic-decorator';
import { bool } from '@ember/object/computed';
import Component from '@ember/component';
import { isEmpty } from '@ember/utils';
import { A } from '@ember/array';
import { inject as service } from '@ember/service';
import { action } from '@ember/object';
import layout from '../templates/components/sb-publisher';
import TreeNode from '../utils/sb-tree-node';
import EmberObject, { set, setProperties, computed } from '@ember/object';
import { allSettled } from 'rsvp';

@classic
export default class SbPublisherComponent extends Component {
  layout = layout;
  classNames = ['sb-publisher'];

  @service store;
  @service settings;
  @service('publish') publishService;
  @service('token') tokenService;
  @service flashMessages;

  selected = A();
  _dropCount = 0;

  init() {
    super.init(...arguments);
    let rawOptions = this.settings.data?.publishOptions;
    let publishOptions = Array.isArray(rawOptions) ? rawOptions : [];
    let settings =
      publishOptions.find(
        (option) =>
          option.catalog === 'ScienceBase' || option.publisher === 'ScienceBase'
      ) || {};
    let config = this.config;
    this.set(
      'treeRoot',
      EmberObject.create({
        label: 'ScienceBase Default',
        icon: 'globe',
        checkable: false,
        isExpanded: true,
        draggable: false,
        notSelectable: true,
        children: null,
        hideToggle: true,
        hideCheck: true,
        isRoot: true,
        nodeClass: 'sb-tree-root',
        sbId: settings['sb-defaultParent'] || config?.defaultParent,
        config: config,
        willDestroy() {
          let children = this.children;
          if (children) {
            children.forEach((itm) => {
              itm.destroy();
            });
          }
        },
      })
    );
  }

  @computed('publishService')
  get config() {
    if (this._config !== undefined) return this._config;
    return this.publishService.catalogs?.findBy('name', 'ScienceBase');
  }

  set config(value) {
    this._config = value;
  }

  @computed('selected', 'selected.[]', 'isPublishing')
  get publishable() {
    if (this.isPublishing) {
      return [];
    }
    return this.selected.filter((itm) => {
      let path = itm.path;
      let length = path?.length;

      return (
        itm.get('sbParentId') ||
        length < 3 ||
        path.objectAt(length - 2).get('isSelected')
      );
    });
  }

  @bool('publishable.length') canPublish;

  @computed('store')
  get records() {
    if (this._records !== undefined) return this._records;
    return this.store.peekAll('record').filter((record) => !record.hasSchemaErrors);
  }

  set records(value) {
    this._records = value;
  }

  @bool('tokenService.token') hasToken;

  @computed(
    'records.@each.recordId',
    'records.@each.parentIds',
    '_dropCount'
  )
  get model() {
    let all = this.records;
    let records = all.filter((record) => !record.hasParent);
    if (isEmpty(records)) {
      return null;
    }
    let tree = records.map((rec) => {
      let node = this.createNode(rec);
      let children = node.addChildren(all);
      if (children) {
        children.forEach((itm) => {
          set(itm, 'parentNode', node);
          itm.addChildren(all);
        });
      }
      return node;
    });
    let treeRoot = this.treeRoot;
    treeRoot.set('children', tree);
    return [treeRoot];
  }

  willDestroyElement() {
    super.willDestroyElement(...arguments);
    let selected = this.selected;
    selected.forEach((itm) => set(itm, 'isSelected', false));
    selected.clear();
    this.treeRoot.destroy();
  }

  createNode(rec) {
    let config = this.config;
    let rawOpts = this.settings.data?.publishOptions;
    let publishOptions = Array.isArray(rawOpts) ? rawOpts : [];
    let settings =
      publishOptions.find(
        (option) =>
          option.catalog === 'ScienceBase' || option.publisher === 'ScienceBase'
      ) || {};
    setProperties(config, {
      defaultParent: settings['sb-defaultParent'] || config?.defaultParent,
      defaultCommunity: settings['sb-defaultCommunity'],
      defaultOrganization: settings['sb-defaultOrganization'],
    });
    return TreeNode.create({
      _record: rec,
      config: config,
      settings: settings,
    });
  }

  @action
  onDropComplete() {
    this.incrementProperty('_dropCount');
  }

  @action
  hash(val) {
    this.set('password', btoa(val));
  }

  @action
  publish() {
    let selected = this.publishable.filter(
      (itm) =>
        itm.path?.length < 3 || !itm.parentNode?.isSelected
    );
    this.set('isPublishing', true);
    const promises = selected.map((record) => {
      set(record, 'isLoading', true);
      return record.publish(this.tokenService.token);
    });
    allSettled(promises).then((results) => {
      set(this, 'isPublishing', false);

      let hasFailure = results.some(
        (result) => result.state === 'rejected'
      );

      if (hasFailure) {
        this.flashMessages.danger('Publishing error!');
      }
    });
  }

  @action
  addToken() {
    const token = this.tokenService.addToken(this.rawToken);
    if (!token) {
      set(this, 'rawToken', null);
    }
  }

  @action
  selectRecord(nodeModel, path) {
    let selected = this.selected;
    let target = selected.findBy('id', nodeModel.id);
    if (
      nodeModel.isSelected &&
      target === undefined &&
      !nodeModel.get('notSelectable')
    ) {
      set(nodeModel, 'path', path);
      selected.pushObject(nodeModel);
    } else {
      selected.removeObject(target);
      set(nodeModel, 'path', []);
      let nodeChildren = nodeModel.get('children') || [];
      nodeChildren.forEach((itm) => {
        if (itm.get('notSelectable') && itm.get('isSelected')) {
          set(itm, 'isSelected', false);
          this.selectRecord(itm);
        }
      });
    }
  }
}
