  ...
  actions: {
    ...
    publish() {
      let selected = this.get('publishable')
        .filter((itm) => (get(itm, 'path.length') < 3 || !get(itm, 'parentNode.isSelected')));
        // publish with keycloak
      //  url: https://sciencebase.gov/auth
      // prod - realm: Sciencebase - client: mdeditor
      // beta - realm: Sciencebase-B - client: mdeditor
    },
