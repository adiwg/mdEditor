...
const {
  computed,
  Component,
  get, getWithDefault,setProperties,
  isEmpty,
  $,
  A,
  set,
...
export default Component.extend({
  ...
  model: computed('records.@each.recordId', 'records.@each.parentIds',
    function() {
      ...
      return [{
        ...
        hideCheck: true,
        isRoot: true,
        sbId: getWithDefault(settings, 'sb-defaultParent', get(config, 'defaultParent')),
        config: config
      }];
    }),
  ...
  createNode(rec) {
    ...
    setProperties(config, {
      defaultParent: getWithDefault(settings, 'sb-defaultParent', get(config,
        'defaultParent')),
