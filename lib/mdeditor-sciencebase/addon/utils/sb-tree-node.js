...
export default EmberObject.extend({
  ...
  sortOrder: computed('config.defaultParent', 'sbParentId', function () {
    ...
    return !parent || this.config?.defaultParent === parent ? 0 : 1;
  }),
  nodeClass: computed('sortOrder', function () {
    return this.sortOrder ? 'tree-node-unrooted' : 'tree-node-rooted';
  }),
  ...
  definition: computed(
    ...
    function () {
      return (
        this._record?.json?.metadata?.resourceInfo?.abstract
          ?.split(' ')
          .splice(0, 50)
          .join(' ') || ''
      );
  ...
  sbParentId: computed(
    ...
    function () {
      let parentIds = this._record?.parentIds;
      ...
      return this.findSbId(this._record?.defaultParent);
    }
  ),
  ...
  sbDate: computed('sbId', function () {
    ...
    let dates = record ? record.authority?.date : null;
    ...
    if (dates && dates.length) {
      ...
      if (published.length === 1) {
        return published.firstObject.date;
      }
  ...
  willDestroy() {
    ...
    if (children) {
      children.forEach((itm) => itm.destroy());
    }
  },
  ...
  publish(refreshToken) {
    ...
    let rootURI = publishOptions?.publisherEndpoint ||
      publishOptions?.['sb-publishEndpoint'] ||
      this.config?.rootURI;
    rootURI = rootURI.replace(/\/$/, '');
    let defaultParent = this.config?.defaultParent;
    let url =
    ...
    let promise = $.ajax(url + urlPut, {
        ...
        if (response.id) {
          ...
          if (idObj) {
            ...
            set(
              idObj,
              'authority.title',
              idObj.authority.title || 'ScienceBase'
            );
            // idObj comes from parsed mdJSON (real citation data or the
            // freshly-defaulted object above), so authority.date is a plain
            // array either way - wrap it in A() so pushObject() below always
            // works, not just when it was just-defaulted to A() ourselves.
            set(idObj, 'authority.date', A(idObj.authority.date || []));

            idObj.authority.date.pushObject({
              date: moment().toISOString(),
              dateType: 'published',
              description: 'Published using mdEditor',
            });
      ...
      (response) => {
        ...
        let xhrError = response.responseJSON?.error?.messages || [];
