...
- **Real scenario coverage, added 2026-09-15/16** (8 tests total, all passing together): Settings persistence (`e2e/settings/persistence.spec.js`), record edit/save/reload (`e2e/record/persistence.spec.js`), Auto Save on/off behavior (`e2e/record/autosave.spec.js`), and contact/dictionary edit/save/reload (`e2e/contact/persistence.spec.js`, `e2e/dictionary/persistence.spec.js`). Shared helpers under `e2e/helpers/` handle the app's splash modal, the Auto Save toggle, and reading a model's raw persisted PouchDB document directly (bypassing ember-data's in-memory state, which reflects an edit immediately regardless of whether it actually saved).
- **Writing the autosave spec surfaced a real, previously-undiscovered app bug**: with Auto Save on, simply leaving a record's edit route mounted causes it to re-save itself roughly every 750ms indefinitely, not save-once-and-settle (root cause traced to `hash-poll.js`'s periodic polling interacting with `app/models/base.js`'s `observeAutoSave`/`hasDirtyHash`, not fully isolated further). Not fixed in this leg — flagged to the user directly and deliberately deferred; see "What's Next" below.
- **Still deferred** (not blocking this PR): Sync push/pull against local CouchDB, and a basic real-boot smoke pass through the migration script.
...
- **Decide what to do about the autosave churn bug** found while writing the E2E suite (see "E2E Testing" above): with Auto Save on, an open record/contact/dictionary edit page re-saves itself every ~750ms indefinitely. It's a real behavioral bug, not something introduced by this leg's changes as far as investigated — worth confirming via `git blame` on `app/models/base.js`'s `hasDirtyHash`/`observeAutoSave` whether it predates this spike, and deciding whether to fix it before or after this PR lands.
- Remaining E2E fast-follows (not blocking this PR): Sync push/pull coverage against local CouchDB, and a migration-script smoke test.
